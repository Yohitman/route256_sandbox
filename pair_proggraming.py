...
for data in range(int(input())):
    quantity = int(input())
    razrabs = list(map(int, input().split()))
    order = list(range(1, quantity+1))
    while razrabs:
        razrab = razrabs[0]
        razrabs.pop(razrabs.index(razrab))
        pair = min(razrabs, key=lambda a: abs(a - razrab))
        pair_pos = razrabs.index(pair)
        print(order[0], order[pair_pos+1])
        order.pop(0)
        order.pop(razrabs.index(pair))
        razrabs.pop(razrabs.index(pair))
    print()
